...
private:
    ...
    map<int, _Type> needles;

public:
    CSearch() = default;
    CSearch(const _Comparator &cmp) : comparator(cmp) {}
...
void CSearch<_Type, _Comparator>::Add(int id, const _Type &needle) {
    needles.insert(pair<int, _Type>(id, needle));
}

template<typename _Type, typename _Comparator>
set<int> CSearch<_Type, _Comparator>::Search(const _Type &hayHeap) const {
    set<int> foundNeedles;
    //typename _Type::const_iterator it
    for (auto it1 = needles.begin(); it1 != needles.end(); it1++ ) {
        auto it = std::search(hayHeap.begin(), hayHeap.end(), it1->second.begin(), it1->second.end(), comparator);
        if ( it != hayHeap.end()) foundNeedles.emplace(it1->first);
    }
    return foundNeedles;
}
...
bool upperCaseCompare(const char &a, const char &b) {
    //  cout << "WHOMST HAS SUMMONED THE ANCIENT ONE" << endl;
